...
void Bureaucrat::executeForm(const Form& f) const{
    ...
    if (f.getExecGrade() < 1)
        throw Form::GradeTooHighException();
    if (f.getExecGrade() > 150) 
        throw Form::GradeTooLowException();
    if (f.getExecGrade() < _grade) {
        std::cout << _name << " couldn't execute " << f.getName() << " because her/his grade is not high enough." << std::endl;
        return ;
    }
    f.executeAction();
    return ;
